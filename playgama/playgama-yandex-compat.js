/* Playgama Bridge v2 compatibility layer for AstroMenace's Yandex-style web integration. */
(() => {
  'use strict';
  ...
  const initializeBridge = async () => {
    ...
    try {
      const markerKey = '__astromenace_playgama_bridge_v2';
      await bridge.storage.get(markerKey).catch(() => undefined);
      await bridge.storage.set(markerKey, { version: 2, updatedAt: Date.now() });
    } catch (error) {
      console.info('[Playgama] storage unavailable; AstroMenace local persistence remains available.', error);
    }
  ...
  const bindInterstitialPause = (bridge) => {
    bridge.advertisement?.on?.(bridge.EVENT_NAME.INTERSTITIAL_STATE_CHANGED, (state) => {
      if (state === bridge.INTERSTITIAL_STATE.OPENED) setPauseReason('interstitial', true);
      else if (state === bridge.INTERSTITIAL_STATE.CLOSED || state === bridge.INTERSTITIAL_STATE.FAILED) {
  ...
  window.playgamaBridgeReady.then((bridge) => {
    if (!bridge) return;
    try { bindInterstitialPause(bridge); }
    catch (error) { console.warn('[Playgama] interstitial lifecycle subscription failed:', error); }
  });
})();
